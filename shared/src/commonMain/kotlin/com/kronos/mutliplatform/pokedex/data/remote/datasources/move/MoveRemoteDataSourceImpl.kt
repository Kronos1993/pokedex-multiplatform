        ...
        val response =
            try {
                httpClient.createKtorClient(httpEngine)
                    .get(urlProvider.getPublicApiUrl() + MoveApi.LIST(limit, offset))
                ...
                return Result.Error(
                    FullNetworkError(
                        ...
                        "No internet connection: ${e.message} - ${
                            urlProvider.getPublicApiUrl() + MoveApi.LIST(limit, offset)
                        }",
                        0
                    )
                )
    ...
    override suspend fun getMove(move: String): Result<MoveInfo, Error> {
        val response =
            try {
                httpClient.createKtorClient(httpEngine)
                    .get(
                        urlProvider.getPublicApiUrl() + MoveApi.GET_MOVE(move)
                    )
            } catch (e: UnresolvedAddressException) {
                e.printStackTrace()
                return Result.Error(
            ...
            in 200..299 -> {
                ...
                if (result.isNotEmpty()) {
                    try {
                        ...
                        val list =
                            json.decodeFromString<MoveInfoDto>(result)
                        Result.Success(list.toMoveInfo())
                    } catch (e: Exception) {
                        e.printStackTrace()
                        Result.Error(
                            FullNetworkError(

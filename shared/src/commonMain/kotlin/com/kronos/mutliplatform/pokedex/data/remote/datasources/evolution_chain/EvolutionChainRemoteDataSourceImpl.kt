...
class EvolutionChainRemoteDataSourceImpl(
    ...
    override suspend fun list(
        ...
        val url = urlProvider.getPublicApiUrl() + EvolutionChainApi.LIST(limit, offset)
        ...
        if (cached != null) {
            return try {
                ...
                e.printStackTrace()
                fetchEvolutionChainListFromNetwork(url)
            }
        }

        return fetchEvolutionChainListFromNetwork(url)
    }

    private suspend fun fetchEvolutionChainListFromNetwork(url: String): Result<ResponseList<NamedResourceApi>, Error> {
        val response =
            try {

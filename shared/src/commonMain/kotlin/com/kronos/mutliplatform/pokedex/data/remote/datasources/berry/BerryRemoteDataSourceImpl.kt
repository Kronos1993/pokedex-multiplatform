...
import com.kronos.mutliplatform.pokedex.data.remote.api.berry.BerryApi
...
class BerryRemoteDataSourceImpl(
    ...
    override suspend fun listBerry(
        ...
        val url = urlProvider.getPublicApiUrl() + BerryApi.LIST(limit, offset)
        ...
        if (cached != null) {
            return try {
                ...
                e.printStackTrace()
                fetchBerryListFromNetwork(url)
            }
        }

        return fetchBerryListFromNetwork(url)
    }

    private suspend fun fetchBerryListFromNetwork(url: String): Result<ResponseList<NamedResourceApi>, Error> {
        val response =
            try {

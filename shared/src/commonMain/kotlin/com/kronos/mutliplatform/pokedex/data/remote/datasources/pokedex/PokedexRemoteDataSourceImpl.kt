...
class PokedexRemoteDataSourceImpl(
    ...
    override suspend fun list(
        ...
        val url = urlProvider.getPublicApiUrl() + PokedexApi.LIST(limit, offset)
        ...
        if (cached != null) {
            return try {
                ...
                e.printStackTrace()
                fetchPokedexListFromNetwork(url)
            }
        }

        return fetchPokedexListFromNetwork(url)
    }

    private suspend fun fetchPokedexListFromNetwork(url: String): Result<ResponseList<NamedResourceApi>, Error> {
        val response =
            try {
    ...
    override suspend fun getPokedex(pokedexId: Int): Result<Pokedex, Error> {
        val url = urlProvider.getPublicApiUrl() + PokedexApi.GET_POKEDEX_BY_ID(pokedexId)
        ...
        if (cached != null) {
            return try {
                ...
                Result.Success(
                    json.decodeFromString<PokedexDto>(cached.response).toPokedex(urlProvider)
                )
            } catch (e: Exception) {
                e.printStackTrace()
                fetchPokedexFromNetwork(url)
            }
        }

        return fetchPokedexFromNetwork(url)
    }

    override suspend fun getPokedex(pokedex: String): Result<Pokedex, Error> {
        val url = urlProvider.getPublicApiUrl() + PokedexApi.GET_POKEDEX_BY_NAME(pokedex)
    ...
    private suspend fun fetchPokedexFromNetwork(url: String): Result<Pokedex, Error> {
        val response = try {
            httpClient.createKtorClient(httpEngine).get(url)
        } catch (e: UnresolvedAddressException) {
            e.printStackTrace()
            ...
            in 200..299 -> {
                ...
                if (result.isNotEmpty()) {
                    try {
                        ...
                        val dto = json.decodeFromString<PokedexDto>(result)
                        ...
                        Result.Success(dto.toPokedex(urlProvider))
                    } catch (e: Exception) {
                        e.printStackTrace()
                        Result.Error(
                            FullNetworkError(

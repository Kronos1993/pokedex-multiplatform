...
| **Security** | `--quick` (default), `--full`, `--security-only` | diff hunks for `.kt`/`.swift`/`.xml` files, `verification.secret_log_keywords` | findings tagged `[code]` for any credential-shaped string (PokeAPI itself is keyless, so any hit here is a new credential) logged, printed, or hardcoded outside its existing config location |
